...
class CityCRUD:
    ...
    async def get_city(self, db: AsyncSession, city_id: int):
        result = await db.execute(select(City).filter(City.id == city_id))
        return result.scalars().first()
    
    async def get_city_by_code(self, db: AsyncSession, code: int):
        result = await db.execute(select(City).filter(City.code == code))
        return result.scalars().first()
    
    async def get_cities(self, db: AsyncSession, skip: int = 0, limit: int = 100):
        result = await db.execute(select(City).offset(skip).limit(limit))
        return result.scalars().all()
    
    async def get_cities_by_level(self, db: AsyncSession, level: int):
        result = await db.execute(select(City).where(City.level == level))
        return result.scalars().all()
    
    async def get_children_cities(self, db: AsyncSession, parent_id: int):
        result = await db.execute(select(City).where(City.parent_id == parent_id))
        return result.scalars().all()
