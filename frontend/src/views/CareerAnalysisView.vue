...
const runStage = computed(
  () => STAGE_LABELS[run.value?.current_node] || (run.value ? "正在准备…" : ""),
);
...
const startRunPolling = (runId, { onComplete, onFailed } = {}) => {
  stopRunPolling();
  ...
  runPollTimer = setInterval(async () => {
    ...
    try {
      ...
      return; // 瞬时网络错误忽略，下一轮继续。
    }
    if (!run.value || String(run.value.id) !== String(runId)) return;
    run.value = next;
...
const generateReport = async () => {
  ...
  try {
    ...
    startRunPolling(runId, {
      ...
      onFailed: (next) => {
        errorMessage.value =
          next.error_message || next.error_code || "职业分析失败，请稍后重试。";
      },
    });
  } catch (error) {
    run.value = null;
...
const pollQuestionRun = (runId, conversationId) => {
  if (questionPollTimer) clearInterval(questionPollTimer);
  questionPollTimer = setInterval(async () => {
    if (!questionPollTimer) return;
    let next;
    try {
      const response = await agentAPI.getRun(runId);
      next = response.data;
    ...
    if (next.status === "completed") {
      ...
      questionPollTimer = null;
      answer.value = "";
      errorMessage.value = next.error_message || next.error_code || "问题分析失败，请稍后重试。";
    }
  }, 3000);
};
...
const sendQuestion = async () => {
  ...
  try {
    ...
    pollQuestionRun(runId, conversationId);
  } catch (error) {
    answer.value = "";
    ...
    <template v-else>
      ...
      <div class="career-layout">
        <div class="career-main">
          ...
          <section v-if="latestReport?.content" class="analysis-card report-card" data-test="latest-report"><div class="section-heading"><div><p>FULL REPORT</p><h2>完整职业分析报告</h2></div><span>{{ latestReport.created_at ? `更新于 ${new Date(latestReport.created_at).toLocaleString("zh-CN")}` : "" }}</span></div><pre class="report-content">{{ latestReport.content }}</pre></section>
        </div>
