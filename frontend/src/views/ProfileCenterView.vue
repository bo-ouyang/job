  ...
  <main class="profile-page">
    ...
    <div class="profile-layout">
      ...
      <div class="profile-content" :aria-busy="loading">
        ...
        <section id="courses" class="profile-card" data-section="courses"><header><div><p>MAJOR COURSES</p><h2>专业课程</h2></div><button @click="addCourse">＋ 添加课程</button></header><div class="item-table course-table"><div class="table-head"><span>课程名称</span><span>分类</span><span>掌握程度</span><span>来源</span><span /></div><div v-for="(item, index) in courses" :key="item.id ?? index"><input v-model="item.name" /><input v-model="item.category" /><select v-model="item.level"><option>了解</option><option>掌握</option><option>熟练</option><option>精通</option></select><span>{{ item.source || "手动添加" }}</span><button aria-label="删除课程" @click="removeItem(courses, index)">×</button></div></div></section>

        <section id="skills" class="profile-card" data-section="skills"><header><div><p>PROFESSIONAL SKILLS</p><h2>专业技能</h2></div><button @click="addSkill">＋ 添加技能</button></header><div class="skill-items"><article v-for="(item, index) in skills" :key="item.id ?? index"><div><input v-model="item.name" /><span>{{ item.category }}</span></div><label><span>熟练度</span><input v-model.number="item.proficiencyLevel" type="range" min="1" max="5" /><b>{{ item.proficiencyLevel }}/5</b></label><p>证据：<input v-model="item.evidence" placeholder="课程、项目或实习" /></p><footer><span>{{ item.source || "manual" }}</span><button @click="removeItem(skills, index)">删除</button></footer></article></div></section>
